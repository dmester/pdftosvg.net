...
{
    internal class TensorProductPatchMeshShading : MeshShading
    {
        public TensorProductPatchMeshShading(PdfDictionary definition, CancellationToken cancellationToken) : base(definition, cancellationToken)
        {
            ReadData();
        }
        ...
        {
            var prevCoordinates = new Point[16];
            var prevColors = new float[4][];

            while (!reader.EndOfInput)
            {
                var coordinates = new Point[16];
                ...
                switch (flag)
                {
                    ...
                    default:
                        Log.WriteLine("Unknown Tensor-Product Patch flag " + flag + ".");
                        return;
                }

                for (var i = keptCoordinates; i < 16; i++)
                {
                    if (!TryReadCoordinate(out coordinates[i]))
                    {
                        Log.WriteLine("Incomplete coordinate in Tensor-Product Patch Mesh shading.");
                        return;
                    }
                }
                ...
                for (var c = keptColors; c < 4; c++)
                {
                    ...
                    {
                        Log.WriteLine("Incomplete color in Tensor-Product Patch Mesh shading.");
                        return;
                    }
                }
