...
{
#if DEBUG
    [DebuggerTypeProxy(typeof(JpegHuffmanTable.DebugProxy))]
    ...
    internal class JpegHuffmanTable
    {
        ...
        public JpegHuffmanTable(ArraySegment<byte> bits, ArraySegment<byte> huffval)
        {
            Bits = bits;
            Huffval = huffval;
            ...
            if (bits.Array != null && huffval.Array != null)
            {
                ...
                for (var size = 1; size <= bits.Count; size++)
                {
                    var count = bits.Array[bits.Offset + size - 1];
        ...
        public static JpegHuffmanTable Empty { get; } = new JpegHuffmanTable(
            ArrayUtils.Empty<byte>(), ArrayUtils.Empty<byte>());

        // Source:
        // K.3.3 Huffman table-specification examples

        public static JpegHuffmanTable DefaultLuminanceDCTable { get; } = new JpegHuffmanTable(
            new byte[]
            {
        ...
        public static JpegHuffmanTable DefaultChrominanceDCTable { get; } = new JpegHuffmanTable(
            new byte[]
            {
        ...
        public static JpegHuffmanTable DefaultLuminanceACTable { get; } = new JpegHuffmanTable(
            new byte[]
            {
        ...
        public static JpegHuffmanTable DefaultChrominanceACTable { get; } = new JpegHuffmanTable(
            new byte[]
            {
        ...
        {
            private readonly JpegHuffmanTable table;

            public DebugProxy(JpegHuffmanTable table)
            {
                this.table = table;
            }
        ...
        private string DebugView => "{ Huffman table, " + encodeTable.Count + " codes }";
#endif
    }
}
